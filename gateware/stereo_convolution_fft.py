...
from fftgen_wrapper import FFTGenWrapper
...
class StereoConvolutionFFT(Elaboratable):
    ...
    def __init__(self,
        ...
        self._tapcount = 128 #len(taps) #4096 synthesizes
        self._bitwidth = bitwidth
        self._convolutionMode = convolutionMode
        self._debug = debug

        self._stepsize = 32  # fftsize-tapcount +1 #or 32
        ...
        self._taps1_memory = Memory(width=self._bitwidth, depth=self._slices*self._fftsize)
        self._taps2_memory = Memory(width=self._bitwidth, depth=self._slices*self._fftsize)
        self._samples1_memory = Memory(width=self._bitwidth, depth=self._slices*self._fftsize)
        self._samples2_memory = Memory(width=self._bitwidth, depth=self._slices*self._fftsize)
        ...
        for j in range(self._slices):
            ...
            for k in range(len(t1)):
                tapsfft1[j * self._fftsize + k] = int(t1[k])
                tapsfft2[j * self._fftsize + k] = int(t2[k])
    ...
    def elaborate(self, platform) -> Module:
        ...
        ix = Signal(range(self._tapcount + 1))

        buffer1 = Array(Signal(signed(self._bitwidth), name=f"buffer1_{i}") for i in range(self._fftsize))
        m.submodules.fft = fft = FFTGenWrapper(self._tapcount, self._bitwidth, 25, False)
