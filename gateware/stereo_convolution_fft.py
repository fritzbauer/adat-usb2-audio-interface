...
class StereoConvolutionFFT(Elaboratable):
    ...
    def __init__(self,
        ...
        self._tapcount = 1024 #len(taps) #4096 synthesizes
        ...
        self._buffer_memory = Memory(width=self._fft_output_bitwidth*2, depth=self._stepsize*2)
    ...
    def elaborate(self, platform) -> Module:
        ...
        madd1 = Signal(self._bitwidth + Shape.cast(range(self._fftsize)).width)

        #gotSamples = Signal(range(self._fft_output_bitwidth))
        #m.d.comb += gotSamples.eq(fft.gotSamples)
        ...
        with m.FSM(reset="READ_SAMPLES"):
            ...
            with m.State("CALC_LEFT_READ_RIGHT_FFT"):
                ...
                channel1_real = samples1_real * taps1_real - samples1_imag * taps1_imag
                channel1_imag = samples1_real * taps1_imag + samples1_imag * taps1_real


                m.d.sync += [
                    madd1.eq(madd1 +
                             samples1_read_port.data[self._bitwidth:self._bitwidth*2] * taps1_read_port.data
                             + samples2_read_port.data * taps2_read_port.data),
            ...
            with m.State("OUTPUT_DATA"):
                ...
                m.d.sync += [
                    ...
                    self.signal_out.payload.eq(buffer_read_port.data >> self._fft_output_bitwidth),
